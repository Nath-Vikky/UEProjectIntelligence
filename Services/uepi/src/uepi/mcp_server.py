...
Treat recovery_required as a hard mutation gate. Inspect the exact marker before choosing recovery. Finalize only when all current package fingerprints match their prepared backups; rollback restores the exact backup set and may overwrite newer bytes. When a later valid transaction makes a prepared marker obsolete, ask the user to explicitly confirm retaining the inspected current disk state, then use uepi_recovery_discard with the exact confirmation token. Never invent or reuse a stale token.
