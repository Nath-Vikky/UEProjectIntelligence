...
Domain handlers own operation-specific preflight and mutation. Actor, Material, and Enhanced Input operations are fully migrated: Registry entries execute exact target/type/path checks, typed probes where applicable, and apply logic. The Bridge supplies transaction context, captures created-asset references, marks Blueprint validation targets, and records the handler result; remaining legacy domains are being moved through the same boundary before Beta.
