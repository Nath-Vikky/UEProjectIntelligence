...
UUEPISettings::UUEPISettings()
	...
	, WriteAuthorizationMode(EUEPIWriteAuthorizationMode::ReviewEachPlan)
	, TrustedProjectBindingId()
	, AllowedAssetRoots({ TEXT("/Game") })
	, AllowedOperationDomains({ TEXT("blueprint"), TEXT("animgraph"), TEXT("animation"), TEXT("asset"), TEXT("actor"), TEXT("content"), TEXT("material"), TEXT("widget"), TEXT("input"), TEXT("config") })
	, MaximumRiskLevel(EUEPIMaximumRiskLevel::High)
	, bAllowAssetDelete(false)
	, bAllowAssetRename(true)
	, bAllowRuntimeControl(true)
	, MaximumAssetsPerTransaction(12)
	, bAlwaysCreateBackup(true)
	, bAlwaysReportAfterApply(true)
	, MaxWriteOperationsPerTransaction(96)
	, MaxWriteAssetsPerTransaction(12)
